...
if "ingested_docs" not in st.session_state:
    st.session_state['ingested_state']={}
...
if thread_docs:
    latest_docs=list(thread_docs.values())
    st.sidebar.success(
...
uploaded_pdf=st.sidebar.file_uploader("Upload PDF for this chat ",type=['pdf'])
if uploaded_pdf:
    if uploaded_pdf.name in thread_docs:
        st.sidebar.info(f"`{uploaded_pdf.name}` already processed for this chat")
    else :
        with st.sidebar.status("Indexing PDF ...", expanded=True) as status_box:
            summary =ingest_pdf(uploaded_pdf.getvalue(),
...
user_input=st.chat_input("Ask about your Documents or tools")
